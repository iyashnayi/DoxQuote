        ...
        <form action="#" method="post">
          <div class="input-group mb-3">
            ...
            <div class="input-group-append">
              <button type="submit" name="btn-search" class="btn btn-outline-secondary bg-light" id="button-addon2"><img src="assets/icon/ic_search_24px.svg"></button>
            </div>
          </div>
        </form>
      <!-------------------------- content---------------------------------------------- -->
      <!-- <div>
        ...
        <?php 
          if (isset($_POST['btn-search'])) {
            $toBeSearched = trim($_POST['search']);
            $search = preg_replace("#[^a-zA-Z]#i", "", $toBeSearched);
            $quoteQuery = "SELECT * FROM MYQUOTE WHERE title LIKE '%$search%' OR writername LIKE '%$search%' ";
            // echo $toBeSearched;
            $total = mysqli_num_rows($quoteData);
            $quoteData = mysqli_query($conn,$quoteQuery) or die("Oops! couldn't search");
            if ($total != 0) { 
